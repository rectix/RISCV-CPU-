...
module exec_unit #(parameter  PC_INIT = `PC_INIT )(
    ...
    logic [`XLEN-1:0]       reg_file_rs0_i,
    logic [`XLEN-1:0]       reg_file_rs1_i,

    //branch unit 

    input  logic              du_branch_flush_o,
    input  logic [`XLEN-1:0]  du_branch_pc_o,
    input  logic              du_branch_taken_i,  //upstream branch taken status

    // DECODE  UNIT
    input  logic              du_stall_o, 
    output logic              du_bubble_i,

    output logic [`ILEN-1:0]    du_instruct_i,
    output logic [`XLEN-1:0]    du_pc_i,



    output logic [4:0]        du_rs0_i,
    output logic [4:0]        du_rs1_i,
    output logic [4:0]        du_rdt_i,

    output logic [6:0]        du_opcode_i,
    output logic [3:0]        du_alu_opcode_i,

    output logic [6:0]        du_func7_i,
    output logic [2:0]        du_func3_i,

    output logic              du_is_R_type_i,
    output logic              du_is_I_type_i,
    output logic              du_is_S_type_i,
    output logic              du_is_B_type_i,
    output logic              du_is_U_type_i,
    output logic              du_is_J_type_i,

    output logic [11:0]       du_immI_i,
    output logic [11:0]       du_immB_i,
    output logic [11:0]       du_immS_i,
    output logic [19:0]       du_immU_i,
    output logic [19:0]       du_immJ_i,
    ...
    input  logic [1:0]                    mem_instruct_type_o,
    input  logic [`ILEN-1:0]              mem_instruct_o,
    input  logic [`XLEN-1:0]              mem_pc_o,



    input  logic [4:0]                    rdt_addr_o,
    input  logic [`XLEN-1:0]              rdt_data_o,
